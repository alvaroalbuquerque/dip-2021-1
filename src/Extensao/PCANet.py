...
class PCANet:
    def __init__(self, k1, k2, L1, L2, block_size, overlapping_radio=0, linear_classifier='svm', spp_parm=None, dim_reduction=None):
        ...
        self.reduction_model = None ##      EDITADO
    ...
    def fit(self, train_data, train_labels):
        ...
        ## EDITADO
        if self.dim_reduction:
            self.reduction_model = PCA(n_components=self.dim_reduction, copy=False)
            self.reduction_model.fit(features, train_labels)
        ## FIM DO EDITADO

        # print(h.heap())
        print('='*20)
        print('features extracted, SVM training')
        self.classifier.fit(self.reduction_model.transform(features), train_labels) # EDITADO
    ...
    def predict(self, test_data):
        ...
        predictions = self.classifier.predict(self.reduction_model.transform(test_features)) ## EDITADO
        print('=' * 20)
        return predictions
